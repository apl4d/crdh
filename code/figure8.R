...
df = read.csv("~/Desktop/subjects/figure8.csv")
...
df$Major.Subject = with(df, reorder(Major.Subject, X1965))
...
sg1 = sg + geom_line(size=0.15) + 
  geom_text(data = subset(dfm, variable == "X1968"), 
...
sg1 = sg1 + geom_text(data = subset(dfm, variable == "X1965"), 
...
ggsave("~/Desktop/subjects/Johnson.pdf",sg1,w=8,h=10,dpi=150)
